...
if __name__ == "__main__":
    ...
    setup(
        ...
        package_dir={"": "src"},
        data_files=[
            (ipath + "/web/css", glob.glob("web/css/*.css")),
            (
                ipath + "/web/img",
                glob.glob("web/img/*.png")
                + glob.glob("web/img/*.jpg")
                + glob.glob("web/img/*.gif"),
            ),
    ...
    if isinstalling:
        ...
        with open(pth, "w") as f:
            f.write("%s/Lib\n" % ipath)
        eggpth = distutils.sysconfig.get_python_lib() + open(source).read()[1:-1]
        os.remove(eggpth)
        os.remove(source)
